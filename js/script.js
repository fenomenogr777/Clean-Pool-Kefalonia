      ...
      function initMaps() {
         ...
         $.getScript(
            ...
            function () {
               ...
               for (var i = 0; i < plugins.maps.length; i++) {
                  var zoom =
                     parseInt(plugins.maps[i].getAttribute('data-zoom'), 10) ||
                     11
                              ...
                              google.maps.event.addListener(
                                 ...
                                 function (markerElement, mapElement) {
                                    ...
                                    for (var k = 0; k < markers.length; k++) {
                                       ...
                                       gMarker =
                                          markers[k].markerElement.gmarker
                                       if (
                                          gMarker.isActive &&
                                          markers[k].markerElement !==
                                             markerElement
            ...
            for (var k = j; k >= -1; k--) {
               if (
                  ...
                  responsive[values[j]]['items'] =
                     k < 0
                        ? 1
                        : parseInt(c.attr('data' + aliaces[k] + 'items'), 10)
               }
               if (
            ...
            $form.attr('novalidate', 'novalidate').ajaxForm({
                        ...
                        $.ajax({
                           ...
                           if (responceCode !== 'CPT000') {
                              if (output.hasClass('snackbars')) {
                                 output.html(
                                    '<p><span class="icon text-middle fa fa-check icon-xxs"></span><span>' +
               ...
               success: function (result) {
                  ...
                  if (result === 'MF000') {
                     ...
                     if (output.hasClass('snackbars')) {
                        output.html(
                           ' <p class="snackbars-left"><span class="icon icon-xxs fa fa-exclamation-triangle text-middle"></span><span>' +
                              msg[result] +
                              '</span></p>'
                        )
                     } else {
